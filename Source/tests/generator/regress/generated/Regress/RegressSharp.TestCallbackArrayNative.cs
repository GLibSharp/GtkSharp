...
#region Autogenerated code
	...
	internal delegate int TestCallbackArrayNative([MarshalAs(UnmanagedType.LPArray, SizeParamIndex=1)]int[] one, UIntPtr one_length, [MarshalAs(UnmanagedType.LPArray, SizeParamIndex=3)]IntPtr[] two, int two_length);
	...
	internal class TestCallbackArrayInvoker {
		...
		{
			ulong cnt_one = (ulong)(one == null ? 0 : one.Length);
			...
			int __result = native_cb (one, new UIntPtr ((uint)cnt_one), native_two, cnt_two);
			return __result;
		}
	}
	...
	internal class TestCallbackArrayWrapper {
		...
		public int NativeCallback ([MarshalAs(UnmanagedType.LPArray, SizeParamIndex=1)]int[] one, UIntPtr one_length, [MarshalAs(UnmanagedType.LPArray, SizeParamIndex=3)]IntPtr[] two, int two_length)
		{
			try {
				int __ret = managed (one, GLib.Marshaller.PtrToStringGFree(two));
				if (release_on_call)
					gch.Free ();
				return __ret;
			} catch (Exception e) {
