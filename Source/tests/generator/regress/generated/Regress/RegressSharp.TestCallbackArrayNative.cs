...
#region Autogenerated code
	...
	internal delegate int TestCallbackArrayNative([MarshalAs(UnmanagedType.LPArray, SizeParamIndex=1)]int[] one, UIntPtr one_length, IntPtr two, int two_length);
	...
	internal class TestCallbackArrayWrapper {
		...
		public int NativeCallback ([MarshalAs(UnmanagedType.LPArray, SizeParamIndex=1)]int[] one, UIntPtr one_length, IntPtr two, int two_length)
		{
			try {
				int __ret = managed (one, GLib.Marshaller.PtrToStringArray (two, two_length, false));
				if (release_on_call)
					gch.Free ();
				return __ret;
			} catch (Exception e) {
